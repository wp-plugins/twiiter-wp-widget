...
class twitter_wp_Widget extends WP_Widget {

	function twitter_wp_Widget() {
		...
		$widget_ops = array(
			'classname' => 'twitter_wp_widget',
			'description' => __('List your last tweet by displaying content, date, and link to follow you', 'tw_wp')
		);

		$control_ops = array();

		$this->WP_Widget('twitter_wp_widget', __('Twitter WP', 'tw_wp'), $widget_ops, $control_ops);
	}
...
add_action('widgets_init', create_function('', 'return register_widget("twitter_wp_Widget");'));
 ...
 */
 if(!is_admin()) {
	...
	function twitter_wp_head() {
		...
		$array_widgetOptions = get_option('widget_twitter_wp_widget');
		...
		if ( $use_default_css )
			...
			$twitter_wp_css .= '<link type="text/css" media="all" rel="stylesheet" id="twitter_wp_widget_styles" href="'. plugins_url(MY_PLUGINBASENAME."/css/twitter_wp.css") . '" />';
